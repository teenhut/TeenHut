...
app.prepare().then(() => {
  ...
  const migrateLegacyHypes = async () => {
    try {
  ...
  // Debug Endpoint
  server.get("/api/debug-users", async (req, res) => {
    try {
  ...
  server.post("/api/hypes", upload.single("file"), async (req, res) => {
    try {
      if (!req.file) {
  ...
  server.post("/api/chat/upload", upload.single("file"), async (req, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ error: "No file uploaded" });
      }
  ...
  // Like Endpoint
  server.post("/api/hypes/:id/like", async (req, res) => {
    try {
      const { id } = req.params;
  ...
  // Comment Endpoint
  server.post("/api/hypes/:id/comment", async (req, res) => {
  ...
  server.get("/api/hypes/:id/comments", async (req, res) => {
    try {
      const { id } = req.params;
      const hype = await Hype.findById(id);
      if (!hype) return res.status(404).json({ error: "Hype not found" });
  ...
  // Get Hypes Endpoint
  server.get("/api/hypes", async (req, res) => {
    try {
      const { userId } = req.query;
  ...
  // View Endpoint
  server.post("/api/hypes/:id/view", async (req, res) => {
    try {
      const { id } = req.params;
  ...
  // Follow Endpoint
  server.post("/api/users/:id/follow", async (req, res) => {
    try {
